...
fn feed_distribution<D: ContinuousCDF<f64, f64>>(
    ...
    for _ in 0..n {
        spine
            .update(&[dist.inverse_cdf(xorshift32(&mut state)) as f32])
            .unwrap();
    }
    spine
}
...
fn all_time_record_shelves_are_exact() {
    ...
    assert_eq!(spine.cell_min(0).unwrap(), values[0]);
    assert_eq!(spine.cell_max(0).unwrap(), values[n - 1]);
}
...
fn zero_counter_and_zero_atom_are_exact() {
    ...
    assert_eq!(spine.zero_count(0).unwrap(), 500);
...
fn mature_secondary_atom_requires_consecutive_tied_batches() {
    ...
    for i in 0..80 {
        spine.update(&[i as f32 + 10.0]).unwrap();
    }
    for _ in 0..16 {
        spine.update(&[7.0]).unwrap();
    }
    assert_eq!(spine.secondary_atom(0).unwrap(), None);

    for _ in 0..16 {
        spine.update(&[7.0]).unwrap();
    }
    assert_eq!(spine.secondary_atom(0).unwrap(), Some((7.0, 16)));
}
...
fn surprise_gain_tracks_an_abrupt_regime_change() {
    ...
    for _ in 0..20_000 {
        spine
            .update(&[normal.inverse_cdf(xorshift32(&mut state)) as f32])
            .unwrap();
    }
    for _ in 0..1_024 {
        spine
            .update(&[(normal.inverse_cdf(xorshift32(&mut state)) + 5.0) as f32])
            .unwrap();
    }
    ...
    assert!(spine.surprise(0).unwrap() > 0.02);
}
...
fn sustained_surprise_restarts_windowed_records() {
    ...
    for _ in 0..3 * config.buffer_capacity {
        spine.update(&[100.0]).unwrap();
    }

    assert_eq!(spine.regime(0).unwrap(), SpineRegime::Restart);
    assert_eq!(spine.recent_min()[0], 100.0);
    assert_eq!(spine.recent_max()[0], 100.0);
    assert!(spine.cell_min(0).unwrap() < 0.0);
}
...
fn finite_memory_adapts_faster_than_no_fading() {
    ...
    for _ in 0..20_000 {
        ...
        fading.update(&[value]).unwrap();
        no_fading.update(&[value]).unwrap();
    }
    for _ in 0..5_120 {
...
fn shared_ruler_selects_linear_and_improves_uniform_accuracy() {
    ...
    for _ in 0..SAMPLES {
        ...
        adaptive.update(&row).unwrap();
        fixed.update(&row).unwrap();
    }
...
#[test]
fn unimplemented_operations_report_unsupported_instead_of_panicking() {
    let mut spine = TensorDigest::<f32, QuantileSpine>::new(&[2]);
    spine.update(&[1.0, 2.0]).unwrap();

    let errors = [
        spine.merge_cells(&[0]).unwrap_err(),
        spine.merge_channels(&[0]).unwrap_err(),
        spine.merge_all().unwrap_err(),
        spine.analyze().unwrap_err(),
        spine.without_zeros().unwrap_err(),
        spine.to_bytes().unwrap_err(),
        TensorDigest::<f32, QuantileSpine>::from_bytes(&[1, 2, 3])
            .err()
            .unwrap(),
    ];
    for error in errors {
        ...
        assert!(
            error
                .to_string()
                .starts_with("QuantileSpine does not implement"),
